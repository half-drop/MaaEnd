...
int wmain() {
    ...
    if (!caps_ok) {
        std::printf("GetPwrCapabilities forwarding failed\n");
        return 3;
    }
    if (role_value < PlatformRoleUnspecified || role_value >= PlatformRoleMaximum) {
        std::printf("Invalid platform role\n");
        return 4;
    }
    if (role_ex_value < PlatformRoleUnspecified || role_ex_value >= PlatformRoleMaximum) {
        std::printf("Invalid platform role ex\n");
        return 5;
    }
