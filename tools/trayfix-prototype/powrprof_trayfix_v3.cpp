...
extern "C" __declspec(dllexport) LONG WINAPI CallNtPowerInformation(
    ...
    return g_nt_power_information(
        information_level, input_buffer, input_buffer_length,
...
extern "C" __declspec(dllexport) POWER_PLATFORM_ROLE WINAPI PowerDeterminePlatformRole() {
    ...
    auto fn = system_powrprof
        ? reinterpret_cast<PowerDeterminePlatformRoleFn>(
              GetProcAddress(system_powrprof, "PowerDeterminePlatformRole"))
        : nullptr;
    if (!fn) {
        AppendTestMarker(L"platform-role-forward-failed");
        return PlatformRoleUnspecified;
    }

    AppendTestMarker(L"platform-role-forwarded");
    return fn();
}

extern "C" __declspec(dllexport) POWER_PLATFORM_ROLE WINAPI PowerDeterminePlatformRoleEx(
    ULONG version) {
    HMODULE system_powrprof = GetSystemPowrProf();
    auto fn = system_powrprof
        ? reinterpret_cast<PowerDeterminePlatformRoleExFn>(
              GetProcAddress(system_powrprof, "PowerDeterminePlatformRoleEx"))
        : nullptr;
    if (!fn) {
        AppendTestMarker(L"platform-role-ex-forward-failed");
        return PlatformRoleUnspecified;
    }

    AppendTestMarker(L"platform-role-ex-forwarded");
    return fn(version);
}
