...
$asm = @()
$asm += 'OPTION CASEMAP:NONE'
$asm += 'EXTERN TrayFixResolveExport:PROC'
$asm += 'EXTERN g_powrprof_exports:QWORD'
$asm += '.code'
...
for ($i = 0; $i -lt $exports.Count; $i++) {
    ...
    $asm += "$stub PROC"
    if ($offset -eq 0) {
        $asm += '    mov rax, QWORD PTR [g_powrprof_exports]'
    } else {
        $asm += "    mov rax, QWORD PTR [g_powrprof_exports+$offset]"
    }
    $asm += '    test rax, rax'
    $asm += "    jnz $ready"
    $asm += ''
